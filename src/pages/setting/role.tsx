...
function DeleteComponent(props: {
  id: number;
  setTableRecord: (obj: object) => void;
  initData: () => void;
}) {
      ...
      <List
        ...
        renderItem={(item: any) => (
          <List.Item
            ...
            style={{
              cursor: 'pointer',
              background: item.title === (tableRecord as any).title ? 'gainsboro' : '',
            }}
            extra={
              <span>
...
function TreeData(props: { tableRecord: any; init: any }) {
  const columns = [
    ...
    },
    {
      ...
      key: 'is_show',
      render: (text: boolean) =>
        !text ? <Tag color="magenta">close</Tag> : <Tag color="green">Open</Tag>,
    },
  ];
  ...
  return (
    <div style={{ width: '78%', display: 'inline-block' }}>
      <Card
        size="small"
        title={
          props.tableRecord.title ? (
            <>
              <span>Current role：</span>
              <h3 style={{ display: 'inline-block' }}>{props.tableRecord.title}</h3>
            </>
          ) : null
        }
        extra={
          <Button
            ...
            disabled={props.tableRecord.title ? false : true}
            type="primary"
            onClick={() => {
              props.tableRecord.rules = selectedRowKeys;
              setTableLoading(true);
              updateRole(props.tableRecord)
                .then((res) => {
                  if (res.code) {

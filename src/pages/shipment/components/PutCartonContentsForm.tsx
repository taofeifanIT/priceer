...
const PutCartonContentsForm = (props: PutCartonContentsFormProps) => {
    const { shipToAddress, carrierName = "other", cartonData, selectedRowKeys, shipmentId, setStepResMsg } = props;
    ...
    const btnGroup = (<div style={{ 'marginBottom': '10px' }}>
        <StepBtn title='Put transportDetails' api={putTransportDetails} params={{ shipmentId, carrierName, treeForm }} callback={(msg) => {
            setStepResMsg(1, msg);
        }} />
        <StepBtn title='Put cartonContent' api={putCartonContents} params={{ shipmentId, carrierName, treeForm, itemDetail }} callback={(msg) => {
            setStepResMsg(1, msg);
        }} />
        <Button
            size='small'
        ...
        <StepBtn title='Submit transport' api={putTransportDetails} params={{ shipmentId, carrierName, treeForm }} callback={(msg) => {
            setStepResMsg(1, msg);
        }} />
        {carrier === 'Amazon partnered' && (<>
            <StepBtn title='Estimate transport' api={estimateTransport} params={{ shipmentId }} ghost callback={(msg) => {
                setStepResMsg(1, msg);
            }} />
            <StepBtn title='Confirm transport' api={confirmTransport} params={{ shipmentId }} ghost callback={(msg) => {
                setStepResMsg(1, msg);
            }} />
            <StepBtn title='Get trackingdetail' api={getTrackingdetail} params={{ shipmentId }} ghost callback={(msg) => {
        ...
        <StepBtn title='Submit Excel' api={updatePackage} params={{ shipmentId, carrierName, treeForm }} callback={(msg) => {
            setStepResMsg(1, msg);
        }} />
        <StepBtn title='Update shipment' api={updateShipment} params={{ shipmentId, carrierName, treeForm }} callback={(msg) => {
            setStepResMsg(1, msg);
        }} />
        <StepBtn title='Get transportDetails' api={getTrackingdetail} params={{ shipmentId }} callback={(msg) => {
            const { data } = JSON.parse(msg);
            try {
                setAmount(data.TransportContent.TransportDetails.PartneredSmallParcelData.PartneredEstimate.Amount);
            } catch (error) {
        ...
        <StepBtn title='Get shipment' api={getShipment} params={{ shipmentId }} callback={(msg) => {
            setStepResMsg(1, msg);
        }} />
    </div>)
    ...
    const ShowAmount = () => {
        if (carrierName === 'Amazon partnered' && amount.CurrencyCode) {
            return <div style={{ 'textAlign': 'center', 'margin': '12 5' }}>
                ...
                <TabPane tab="Excel" key="2">
                    <>
                        ...
                        <Space style={{ 'marginBottom': '10px' }}>
                            ...
                            <Input.Group compact>
                                <InputNumber style={{ 'width': '140px' }} value={carton_number} onChange={(val: any) => setCarton_number(val)} />
